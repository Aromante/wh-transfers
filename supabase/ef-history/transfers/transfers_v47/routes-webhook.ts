...
export async function handleShopifyTransferWebhook(req: Request, env: Env) {
    ...
    const apiVer = String(env.SHOPIFY_API_VERSION || '2025-10')
    const transferData = await fetchShopifyTransfer(env, apiVer, gid)

    // ── CANCELLED handling ──
    if (transferData.status === 'CANCELLED') {
        // Find internal transfer and cancel if still pending
        const cancelRows = await sbSelect(env, 'transfer_summary',
            `shopify_transfer_id=eq.${encodeURIComponent(gid)}&select=transfer_id,status`)
        const cancelTarget = cancelRows?.[0]
        if (cancelTarget && cancelTarget.status === 'pending') {
            await sbUpdateTransferById(env, cancelTarget.transfer_id, { status: 'cancelled' })
            await sbLogTransfer(env, cancelTarget.transfer_id, 'cancelled_by_shopify_webhook', { shopify_transfer_id: gid })
            return { data: { transfer_id: cancelTarget.transfer_id, cancelled: true, shopify_transfer_id: gid } }
        }
        return {
            data: {
                skipped: true,
                reason: cancelTarget ? `status=${cancelTarget.status} — no se puede cancelar` : 'No hay transfer interno asociado',
