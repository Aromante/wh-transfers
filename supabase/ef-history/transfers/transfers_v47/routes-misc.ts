...
import { type Env, EF_VERSION } from './helpers.ts'
import { sbSelect, sbLogTransfer, sbGetTransferById, sbListLocations, resolveBox } from './supabase-helpers.ts'
...
export async function handleHistory(req: Request, env: Env) {
    ...
    let q = `select=transfer_id,origin_id,dest_id,origin_odoo_id,dest_odoo_id,status,odoo_transfer_id,odoo_picking_id,shopify_transfer_id,created_at,sku_count,total_units,lines&order=created_at.desc&limit=${limit}&offset=${offset}`
...
export async function handleHistoryCSV(req: Request, env: Env) {
    ...
    const headers = ['transfer_id', 'origin_id', 'dest_id', 'origin_odoo_id', 'dest_odoo_id', 'status', 'odoo_transfer_id', 'odoo_picking_id', 'shopify_transfer_id', 'sku_count', 'total_units', 'created_at']
...
export async function handleHealth(env: Env) {
    const checks: Record<string, any> = { ok: true, version: EF_VERSION, ts: new Date().toISOString() }
    // Odoo check
    try {
    ...
    if (env.SHOPIFY_DOMAIN && env.SHOPIFY_ACCESS_TOKEN) {
        try {
            const ver = String(env.SHOPIFY_API_VERSION || '2025-10')
...
export async function handleAdjustPlantaInventory(req: Request, env: Env) {
    ...
    await adjustShopifyPlantaInventory(
        ...
        itemQtyMap,
        async (event, data) => {
            await sbLogTransfer(env, 'manual-adjust', 'shopify_planta_manual_adjust', {
                sub_event: event, ...data, skus: parsed.map(l => l.sku), lines: parsed.length,
            })
        },
