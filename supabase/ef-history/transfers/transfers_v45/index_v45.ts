// Supabase Edge Function: transfers v45
import { serve } from 'https://deno.land/std@0.168.0/http/server.ts'
import { type Env, corsHeaders, json, mustEnv, validateApiKey } from './helpers.ts'
...
function envFromDeno(): Env {
    ...
    return {
        ...
        SHOPIFY_API_VERSION: g('SHOPIFY_API_VERSION', '2025-10'),
        SHOPIFY_CONQUISTA_LOCATION_ID: g('SHOPIFY_CONQUISTA_LOCATION_ID'),
        SHOPIFY_API_KEY_TRANSFERS: g('SHOPIFY_API_KEY_TRANSFERS'),
    }
}
...
serve(async (req: Request) => {
    ...
    try {
        ...
        // ── API Key auth — exempt: /health (public), /webhook/* (Shopify-originated) ──
        const isExempt = path === '/health' || path.startsWith('/webhook/')
        if (!isExempt) {
            const authErr = validateApiKey(req, env, cors)
        ...
        const body = result?.error ? { error: result.error, ...(result?.data || {}) } : (result?.data ?? result)
